...
describe("parseAlertPushConfig", () => {
  ...
  it("没配置过时默认关闭——新功能不该自己开始往群里发消息", () => {
    // 触发线现在是 70（见 types.ts ALERT_TRIGGER_SCORE 的注释，原 80）。
    expect(parseAlertPushConfig(null)).toEqual({ enabled: false, minScore: 70 });
  });

  it("minScore 低于触发线时抬回触发线，低了也不会有更多警报", () => {
    expect(parseAlertPushConfig({ enabled: true, minScore: 50 }).minScore).toBe(70);
  });
