...
| **L3a** | 主语言（zh-CN）成功 | 另一语用**同一个模型**逐字段翻译主语言稿，译文重跑整套质量门槛（以原稿为数字基准）。这是英文版的主路 |
| **L3b** | L3a 失败 | 回落到 Google Translate 免费端点逐字段翻译。该端点无鉴权、对数据中心 IP 段整体拦截（命中即 429，与频率无关），只能当备胎——它曾是英文版反复降级的根因 |
