...
describe("PIVOT_N", () => {
  it("保持为 2——确认滞后 1 小时；这个值同时决定本文件夹具的结构，改动前先改夹具", () => {
    // 从 5 降到 2 是为了把确认滞后从 2.5 小时压到 1 小时。副作用是摆动点
    // 变多变噪，而且 oiDivergence 的两条阈值是在 5 的前提下量出来的，
    // 分数会系统性偏低——这条断言存在的意义就是让下一次改动必须先看到这段。
    expect(PIVOT_N).toBe(2);
  });
});
