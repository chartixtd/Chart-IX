...
~~实测 120 并发全部 200、耗时 2.48 秒。上游不是瓶颈。~~ **（2026-08-18 用真实 key 跑 dryrun 后证伪，见下）**

真实约束来自响应头：

```
API-KEY-MAX-LIMIT: 80
API-KEY-USE-LIMIT: <本分钟已用>
```

**每分钟 80 次请求。** 约束是速率，不是并发数。当初那次压测为什么给出了错误结论：
它只打了一轮 120 个并发请求，单次爆发没触发速率限制（CoinGlass 大概率是按滚动分钟窗口计数，
而不是限制瞬时并发），所以看起来「并发随便开、上游不是瓶颈」。但流水线一轮原本要打 450–800 次
调用，无论并发数开多大，累计请求数在几秒内就会撞穿这条 80/分钟的线，之后的请求全部收到
...
GitHub Actions cron-tick (*/5)
  └→ GET /api/cron/screener-scan   (沿用 authorizeCronTick)
       ...
       └ pipeline.run()
           │
           ├ ① 批量层｜4 路并行，2 次 CoinGlass 调用
           │    BingX getFuturesTickers()      → 可交易白名单 + 24h 高低（振幅），0 次 CG 调用
           │    CoinGecko fetchMarketCapRows() → 市值 + 排名，0 次 CG 调用
           │    CG liquidation/coin-list       → 全币 1h/4h/12h/24h 爆仓，1 次
           │    CG funding-rate/exchange-list  → 全币资金费率，1 次
           │
           ├ ② 粗筛｜0 次调用
           │  BingX 可交易 ∩ 非合成品 ∩ CoinGecko 排名 >50 ∩ 市值 20M–800M
           │  ∩ BingX 24h 振幅 ≥0.5% ∩ BingX quoteVolume ≥2M（挡真正没成交的币）
           │  → 池子大小不固定，视市场行情而定
           │
           ├ ③ 预排序｜0 次调用
...
「上游不是瓶颈」这个后来被证伪的前提下设计的——粗筛池子常有 100–150 个候选，
...
- 市值 20M – 800M
- BingX 24h 振幅 ≥ 0.5%
- BingX `quoteVolume` ≥ 2M（T19 新增，替换了原来的 CoinGlass `volume_usd` ≥ 5M——
  那个门槛已删除，见上面「数据流」一节「为什么是预排序」的解释。
  这个新门槛定在 BingX 长尾假数据带（619–691 万）下方，只挡真正没有成交的币，
  不是真正的流动性判断，真流动性判断交给客户端滑块用 CoinGlass 真实值来做）
...
两边共用同一组常量，并由一个断言测试钉死这个包含关系（T19 之后这条关系只剩市值与振幅两项——
成交额那一项已经不是「两边同源取等值」的关系了：服务端粗筛用的是 BingX `quoteVolume`
的粗粒度门槛，只挡真正没有成交的假带；客户端滑块用的是 CoinGlass 真实 `volume_usd`，
两者不再需要对齐，滑块的成交量档位本身就是唯一的流动性判断）。
...
- ~~两段式「粗筛 → 精算 Top N」的成本优化 —— 已评估并否决（省下的成本买不到东西，
  ...
  **T19 更新：这条否决已被采纳翻案。** 否决它的前提是「上游不是瓶颈、调用量充足」，
  这个前提被真实 dryrun 证伪——`API-KEY-MAX-LIMIT: 80`/分钟下，三段式一轮要打
  450–800 次调用，超配额一个数量级，不是「省成本」的可选优化，而是不这么做就
  跑不通。当初担心的「用单因子预分决定谁有资格参加四因子评分」这个逻辑漏洞确实存在
  （见「数据流」一节「预排序的代价」），但两害相权，这是配额约束下唯一可行的选择
