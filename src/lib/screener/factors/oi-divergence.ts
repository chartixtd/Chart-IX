...
/**
 * 摆动点识别的半窗宽度：左右各这么多根都不更极端，才算一个已确认的摆动点。
 *
 * **1 根 30 分钟 = 确认滞后 30 分钟**，也就是这个粒度下的理论下限：再小就
 * 没有「左右两侧」可言了。此前是 5（滞后 2.5 小时）、后来 2（1 小时），
 * 一路压到 1 是同一个理由——这套系统的目标是「大动作刚启动时抓到」，
 * 确认延迟跟这个目标直接冲突。
 *
 * 代价要说清楚，两条：
 *
 * 1. **摆动点会多很多，也更噪。** 半窗 1 根意味着「比左右两根都极端」就算
 *    一个结构——日内任何一次小回抽都会被认成摆动点。原来 7 天 336 根平均
 *    出 21 个高点，1 根之后是数量级上的差别。
 *
 *    连带影响两处：sweep 找的「前低」会是很近的一个局部低点（findSweep 要求
 *    结构位成形于 sweep 那根之前 n 根，n=1 时只隔 2 根），A2/B2 因此更容易
 *    触发但每次的结构位更浅；六场景「创新极值」那道门也更容易过。
 *
 * 2. **打分口径跟着变了。** oiDivergence 也用这个常量，而它的两条阈值
 *    （OI_DIFF_MIN_PCT / OI_DIFF_FULL_PCT）是在 5 的前提下量出来的：
 *    相邻摆动点挨得更近 → 两点之间的 OI 变化更小 → 分数系统性偏低，
 *    而且 1 比 2 更严重。要把打分留在旧口径上，就得把这里拆成两个常量
 *    （场景用 1、打分用 5）。
 *    没有拆，是因为两个「摆动点」用不同定义会让人读代码时更容易搞错。
 */
export const PIVOT_N = 1;
