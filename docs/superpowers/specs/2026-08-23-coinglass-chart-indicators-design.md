...
用户并排一看：CoinGlass 的 OI 对话框只有 Bar Highlighter + 交易所勾选，我们却多出
品种来源/保证金类型/单位/显示方式，于是怀疑算法也没对齐。逐项拆开：

| 差异 | 影响数值吗 | 说明 |
|---|---|---|
| 它们按保证金类型拆成**三个独立指标**，我们是一个指标 + 下拉 | ❌ | 选到同一档时打的是同一个端点 |
| 它们 OI 不暴露 Symbol / Measure | ❌ | 它们写死「主图品种 + 美元」，正是我们的默认值 |
| 我们多了「显示方式（蜡烛/折线）」 | ❌ | 它们放在 Style 页 |
| 我们没有 Bar Highlighter | ❌ | 只给满足阈值的 bar 上色，Condition=No Filter 时完全不动 |
| **默认勾选的交易所不同** | ✅ | 见下 |

**OI 侧根本没有"算法"**：CoinGlass 的三个 OI 端点直接返回 OHLC，我们只做毫秒转秒、
字符串转数字、去重排序、按时间戳对齐。中间没有任何计算，也就没有算错的余地
（实测同一时刻 2.318B vs 2.317B）。OI 的数值只由五样东西决定：币种、周期、
保证金类型、单位、交易所集合。

所以唯一要修的是最后一项：`DEFAULT_EXCHANGES` 里 OI 那两组此前是我按「主流所」
自己拟的 6–7 家（Bitget/Gate/HTX/Bitmex 都在内），而 CoinGlass 的 OI 对话框默认只
勾 Binance/Bybit/OKX。多算几家交易所，同样口径下数值当然对不上。已改成照抄它们的
勾选，并加了一条测试钉住。

**尚未确认**：用户那张 OI 对话框截图只到 Bitget，再往下（CoinEx / Gate /
Crypto.com / Bitunix / Hyperliquid / MEXC / KuCoin / LBank / WhiteBIT）看不到。
