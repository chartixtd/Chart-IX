 ...
 * PoolScan.bars），不占 CoinGlass 的深扫配额。名额现在的作用只剩一个：让有
 ...
 */
/**
 * 一张卡最多活多久。**这是两个失效条件里的第二个**，另一个是价格碰到失效线，
 * 两者谁先到算谁（见 cards.ts advanceCards）。
 *
 * 6 小时这条线跟信号本身的时效对得上：这类结构事件的生命周期实测是几十分钟
 * 到几小时，再叠上 30 分钟的确认延迟（场景锚在已确认的摆动点上），超过这个
 * 长度的卡已经从「入场信号」退化成「趋势确认」——还有参考价值，但不该再照着
 * 它进场，而卡片上那句操作指令说的正是「照着进场」。
 *
 * 跟碰线那条的分工：碰线回答「市场证伪了它」，超时回答「它已经过气了」。
 * 一个信号可以两样都不占（继续活着），但只要占一样就该撤下来。
 *
 * 从 firstSeenAt 算起，也就是**卡片出现**那一刻，不是结构成形那一刻。后者
 * 可以是一天前的事（结构锚点），拿它计时会让不少卡一出生就超时。
 */
export const CARD_MAX_AGE_MS = 6 * 60 * 60 * 1000;

/**
 * 价格碰到失效线之后，卡片还灰着留多久。**从失效那一刻算起**
 * （AlertCardData.expiredAt），不是从卡片出现算起。
 *
...
/** 最多同时留几张已失效的卡。防止行情剧烈时警报栏被灰卡淹掉。 */
export const CARD_GRACE_MAX = 12;

/**
 * 最多同时挂几张活卡。**这是安全阀，不是失效条件。**
 *
 * 卡片只会因为碰线或超时而结束（见 cards.ts 顶部那段），不会因为场景不再
 * 成立而消失。CARD_MAX_AGE_MS 已经给了每张卡一个硬性上限，所以堆积的风险
 * 比没有超时那会儿小得多；这条线留着只是让 payload 有个已知的天花板。
 *
 * 60 是按实测估的：每轮新判出 3–4 个场景，一张卡最多活 6 小时（24 轮扫描），
 * 稳态实测在 10–30 张之间。真撞到 60 会 warn——那时该查的是新卡为什么突然
 * 变多，而不是把这个数字调大。
 */
...
export interface ScannerPayload {
  ...
  rows: ScannerRow[];
  /**
   * 警报卡片，活着的在前（按总分从高到低），已失效的沉底。
   *
   * **它是独立的实体，不是 rows 的视图。** 一张卡从信号出现开始活着，一直到
   * 价格碰到它自己的失效线为止；这中间即使系统判不出这个场景了、即使这个币
   * 掉出了扫描名单，它照样在。完整语义见 cards.ts 顶部那段。
   *
   * 所以这里会出现「rows 里那一行没有场景，而这个币的卡还在」——两者回答的
   * 不是同一个问题：表格说的是这个币现在是什么局面，卡片说的是那个还没被
   * 证伪的信号。
   */
  cards: AlertCardData[];
  /**
