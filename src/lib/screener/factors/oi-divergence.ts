 ...
 *
 * **2 根 30 分钟 = 确认滞后 1 小时。** 此前是 5（滞后 2.5 小时）——那意味着
 * 结构真的翻掉之后还要等两个半小时才判得出来，而这套系统的目标是「大动作
 * 刚启动时抓到」，2.5 小时的延迟跟这个目标是直接冲突的。
 *
 * 代价要说清楚，两条：
 *
 * 1. **摆动点会多很多，也更噪。** 半窗越窄，越小的波动越容易被认成结构。
 *    原来 7 天 336 根平均出 21 个高点，2 根之后会显著更多。
 *
 * 2. **打分口径跟着变了。** oiDivergence 也用这个常量，而它的两条阈值
 *    （OI_DIFF_MIN_PCT / OI_DIFF_FULL_PCT）是在 5 的前提下量出来的：
 *    相邻摆动点挨得更近 → 两点之间的 OI 变化更小 → 分数系统性偏低。
 *    要把打分留在旧口径上，就得把这里拆成两个常量（场景用 2、打分用 5）。
 *    没有拆，是因为两个「摆动点」用不同定义会让人读代码时更容易搞错。
 */
export const PIVOT_N = 2;
