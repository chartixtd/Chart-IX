...
export function RiskEditor({ configs }: RiskEditorProps) {
  ...
  const saveTier = async (tier: "free" | "pro", state: TierFormState, setState: (s: TierFormState) => void) => {
    ...
    try {
      ...
      if (res.ok) {
        setState({ ...state, saving: false, message: { type: "success", text: "Saved successfully" } });
        router.refresh();
      } else {
        setState({ ...state, saving: false, message: { type: "error", text: json.error ?? "Save failed" } });
      }
    } catch {
      setState({ ...state, saving: false, message: { type: "error", text: "Network error" } });
    }
  };
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card padding="lg">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-text-primary">Free Tier</h2>
            <p className="text-sm text-text-muted">Basic risk limits for free users</p>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            />
            <p className="text-xs text-text-muted">Comma-separated trading pairs</p>
          </div>
      ...
      <Card padding="lg">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gold">Pro Tier</h2>
            <p className="text-sm text-text-muted">Advanced risk limits for pro users</p>
          </div>
        ...
        <div className="space-y-4">
          ...
          <Input
            label="Max Leverage"
            type="number"
            hint="Pro tier should have significantly higher leverage than free tier"
            placeholder="e.g. 125"
