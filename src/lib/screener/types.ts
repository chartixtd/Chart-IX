 ...
 *
 * 卡片现在只会因为价格碰到失效线而结束（见 cards.ts 顶部那段），没有时间
 * 上限、也不会因为场景不再成立而消失，所以理论上它们可以一直堆下去。这条线
 * 存在的唯一目的是让 payload 有个已知的天花板，别把缓存和前端撑爆。
 *
 * 60 是按实测估的：每轮新判出 3–4 个场景，而绝大多数卡在几小时内会碰线，
 * 稳态实测在 10–30 张之间。真撞到 60 会 warn——那时该查的是「是不是有一批
 * 卡的失效线画得太远」，而不是把这个数字调大。
 */
export const CARD_MAX_LIVE = 60;

export const SCANNER_PAYLOAD_VERSION = 11;
